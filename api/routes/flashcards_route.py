...
from api.api_schemas import (
    ...
    BulkOperationResult, AnswerType
)
    ...
    try:
        ...
        if quiz_id:
            ...
            raise HTTPException(
                ...
                detail="quiz_id parameter is required"
            )
        ...
        if filters.answer_type:
            filtered_cards = [c for c in filtered_cards
                            if getattr(c, 'answer_type', 'text') == filters.answer_type.value]

        if filters.search_text:
            search_lower = filters.search_text.lower()
            filtered_cards = [c for c in filtered_cards
                            if search_lower in c.question_text.lower() or
                               search_lower in c.answer_text.lower()]
        ...
        return create_response(
            data=card_data,
            message=f"Retrieved {len(card_data)} flashcards"
        )
        ...
        raise HTTPException(
            ...
            detail=f"Failed to retrieve flashcards: {str(e)}"
        )
...
async def get_flashcard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve flashcard: {str(e)}"
        )
...
async def create_flashcard(
    ...
    try:
        ...
        if not quiz:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Quiz with ID {flashcard_data.quiz_id} not found"
            )
...
async def bulk_create_flashcards(
    ...
    try:
        ...
        if not quiz:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Quiz with ID {bulk_data.quiz_id} not found"
            )
        ...
        result = BulkOperationResult(
            total=len(bulk_data.flashcards),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to bulk create flashcards: {str(e)}"
        )
...
async def get_random_flashcards(
    ...
    try:
        ...
        return create_response(
            data=card_data,
            message=f"Retrieved {len(card_data)} random flashcards"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get random flashcards: {str(e)}"
        )
...
async def get_flashcard_answer_types(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get answer type statistics: {str(e)}"
        )