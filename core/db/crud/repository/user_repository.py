...
from sqlalchemy import and_, desc, func, select  # pylint: disable=import-error
...
class UserRepository(BaseRepository[models.User]):
    ...
    def get_by_name(self, name: str) -> Optional[models.User]:
        ...
        """
        return self._execute_single(select(models.User).where(func.lower(models.User.name) == func.lower(name)))
    ...
    def get_by_email(self, email: str) -> Optional[models.User]:
        ...
        Returns:
            User instance or None if not found
        """
        return self._execute_single(select(models.User).where(func.lower(models.User.email) == func.lower(email)))
    ...
    def get_users_with_sessions(self) -> Sequence[models.User]:
        ...
        """
        return self._execute_query(select(models.User).join(models.Session).distinct().order_by(models.User.name))

    def get_most_active_users(self, limit: int = 10, days: Optional[int] = None) -> Sequence[Tuple[models.User, int]]:
        """
        ...
        """
        query = select(models.User, func.count(models.Session.id).label("session_count")).join(models.Session)
        ...
        query = query.group_by(models.User.id).order_by(desc(func.count(models.Session.id))).limit(limit)
    ...
    def get_users_by_registration_date(
        ...
        return self._execute_query(query.order_by(desc(models.User.created_at)))
    ...
    def delete_user_and_sessions(self, user_id: int) -> bool:
        ...
        self.db.query(models.Session).filter(models.Session.user_id == user_id).delete()
    ...
    def get_user_statistics_summary(self) -> dict:
        ...
        users_with_sessions = self.db.scalar(
            select(func.count(func.distinct(models.Session.user_id))).select_from(models.Session)
        )

        # Users registered in last 30 days
        thirty_days_ago = datetime.now() - timedelta(days=30)
        recent_registrations = self.db.scalar(
            select(func.count()).select_from(models.User).where(models.User.created_at >= thirty_days_ago)
        )
